...
const DueTodaySection: React.FC<Props> = ({ items, totalCount, onEventPress, onViewMore }) => {
  const remaining = totalCount - items.length

  return (
    <View>
      <SectionHeader title="Due Today" count={totalCount} variant="amber" onCountPress={onViewMore} />
      {items.map(item => (
        <CareEventCard
          key={item.event.id}
